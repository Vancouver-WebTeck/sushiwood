...
const Menu = () => {
	...
	if (selectedLocation === 'Nelson, BC') {
		...
		if (menuOption === 'food') {
			...
			menuImageArr = ['sushiwood/fernie-menu/drink-menu/drink-menu-01.webp', 'sushiwood/fernie-menu/drink-menu/drink-menu-02.webp'];
		}
	}
		...
		<MaxWidthContent className='flex items-center justify-center flex-col gap-10 px-14'>
			...
			{selectedLocation === 'Fernie, BC' && (
				<div className='flex gap-4'>
					<button className={`${menuOption == 'food' && 'underline'}`} onClick={() => setMenuOption('food')}>
						Food Menu
					</button>
					<button className={`${menuOption == 'drink' && 'underline'}`} onClick={() => setMenuOption('drink')}>
						Drink Menu
					</button>
				</div>
			)}
			...
			<div className='block md:hidden mt-4'>
				<button onClick={handleModalToggle} className='px-4 py-2 bg-white text-black rounded-md'>
					Open Menu
				</button>
			</div>
			...
			>
				{menuImageArr.map((el, index) => (
					<SwiperSlide key={index} className='relative max-w-xl'>
						<Image src={`${el}`} alt={'menu'} height={3300} width={2550} style={{ objectFit: 'cover' }} unoptimized />
					</SwiperSlide>
				))}
			</Swiper>
			...
			{isModalOpen && (
				...
				>
					...
					>
						...
						>
							<span className='mr-2'>Close</span> {/* Add margin-right to space it from the icon */}
							<SidebarClose /> {/* Assuming this is an icon or component for closing */}
						</button>
					</div>
				</div>
			)}
