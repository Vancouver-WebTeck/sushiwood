...
const formSchema = z.object({
	...
	phoneNumber: z
		...
		.refine((value) => canadianPhoneNumberRegex.test(value), 'Invalid Canadian phone number. Please use a valid format (e.g., (123) 456-7890).'),
...
const Calendar = ({ classname }: { classname?: string }) => {
	...
	return (
		...
		>
			...
			<Form {...form}>
				<form onSubmit={form.handleSubmit(onSubmit)} className='p-4 flex flex-col mx-auto h-full justify-center space-y-6 w-72 md:w-96'>
					<h2 className='text-center text-2xl font-medium'>Book your reservations</h2>
							...
							<FormItem className='flex flex-col'>
								...
								<Popover>
									<PopoverTrigger asChild>
										<FormControl>
											<Button variant={'default'} className={cn(' pl-3 text-left font-normal', !field.value && 'text-muted-foreground')}>
									...
									<PopoverContent className='w-auto p-0' align='start'>
										<Cal mode='single' selected={field.value} onSelect={field.onChange} fromDate={new Date()} initialFocus />
									</PopoverContent>
								</Popover>
								<FormMessage />
							</FormItem>
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								...
								<FormControl>
									<Input type='number' className='bg-primary text-white' placeholder='1' {...field} />
								</FormControl>
								<FormMessage />
							</FormItem>
						)}
